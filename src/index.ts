...
export function locateChromeOrExplain(options?: boolean | { allowFallback?: boolean }): string {
  const allowFallback = typeof options === 'boolean' ? options : Boolean(options?.allowFallback);
  ...
  if (typeof found === 'string' && found) {
    ...
    if (looksOfficialChrome(versionLine) && typeof major === 'number' && major >= 137) {
      found = null as any;
    }
  }
...
function resolveFromPuppeteerBrowsersCLI(): string | null {
  try {
    const out = execFileSync('npx', ['-y', '@puppeteer/browsers', 'path', 'chrome@stable'], {
      encoding: 'utf8',
